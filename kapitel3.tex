...
%mention fully connected layers
...
Between the input and output lay three hidden layers with five, three and again five hidden units.
...
\begin{gathered}
\mathbf{\hat{Y}} = \mathbf{W_O(W_3(W_2(W_1X)))}
\end{gathered}
\end{equation}
...
This simple network only contains three hidden layers with at most five hidden, but still X computations are needed for the forward propagation of a single input vector.
In practice deep learning models contain often more than Y layers, leading to high computational demands.
