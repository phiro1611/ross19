...
The figure \ref{fig:EdgePrepro} shows a steady increase of $Latency_{preprocessing}$ and $Memory_{preprocessing}$ across the increasing image sizes.
...
\textbf{resizing biggest impact, not normalisation}
...
negligible CPU usage differences, but probably not in GPU, which we can not report in this thesis.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $10$x slower $Latency_{preprocessing}$
        \item $1.5$x more $Memory_{preprocessing}$
    \end{itemize}
    \item NNAPI leads to way better performance across all models:
    \begin{itemize}
        \item $4$x faster $Latency_{inference}$
        \item $1.2$x less $Memory_{inference}$
        \item $3$x less $CPU_{inference}$
    \end{itemize}
    \item In comparison to InceptionV4 MobileNetV2 quantized consumes
    \begin{itemize}
        \item $20$x faster $Latency_{inference}$
        \item $2.2$x more $Memory_{infernece}$
    \end{itemize}
\end{itemize}}
...
\emph{
\begin{itemize}
    \item Cloud Preprocessing bottleneck for large image sizes, but faster for very small image sizes
    \item Server latency bottleneck, not network fast fast networks
\end{itemize}
}
