...
This module is visualised in figure \ref{fig:bottleneckBlock} and consists of two parts: The inverted residual block and a shortcut from the input to the output.%Mention activation functions?
...
The general architecture of the network can be seen in figure \ref{fig:inceptionv4Archi} with varying Inception (A-C) and Reduction(A-B) building blocks. 
The intuition behind the Inception modules is 
These Stem, Inception and Reduction modules consists of multiple convolutional, pooling and filter concat layers. 
...
\begin{subfigure}[b]{.95\textwidth}
   \input{Bilder/InceptionAModule.tex}
...
\begin{subfigure}[b]{.95\textwidth}
   ...
   \input{Bilder/InceptionReductionModule.tex}
   \caption{Inception-Reduction module}
...
\begin{figure}
\centering
   \resizebox{.9\linewidth}{!}{\input{Bilder/InceptionAModule.tex}}
\caption{Inception-A module}
...
\begin{figure}
\centering
   \resizebox{.7\linewidth}{!}{\input{Bilder/InceptionReductionModule.tex}}
\caption{Inception-Reduction module}
\label{fig:InceptionReduction}
%%NO DROPOUT AT INFERENCE
\end{figure}
%%Sinn hinter einzelenes Modulen kurz erklären
%%Intuition behind inception blocks
%paralll conv layer, then concat
%%ein inception und ein reduction module erklären
%multiple convolutions with multiple filters and pooling layers simultaneously in parallel within the same layer 
%do all the convolutions and let the model pick what’s best.


Besides dropout InceptionV4 also benefits from the use of batch normalisation during training.
%\begin{figure}[]
%\centering
