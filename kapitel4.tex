...
This growing gap is explained by the fact that for cloud preprocessing only encoded \emph{PNG} images needs to be loaded, but for edge preprocessing these images needed to be encoded, hence more memory is needed.
...
For cloud preprocessing there is no difference in memory consumption between the difference models, but for edge preprocessing InceptionV4 tends to use $x\%$ more memory, because of the models larger input size (InceptionV4 $299^2$, MobileNetV2 $224^2$).
...
Similar to the $Memory_{preprocessing}$ results, there is little difference between the different models, but large differences between the preprocessing methods for increasing image and batch sizes.



The are not significant difference between the examined model types.
\begin{figure}[!htb]
\centering
...
Edge inference consumes substantially more than the cloud inference counter-parts with InceptionV4 consuming $1289$MB for a batch size of $16$ and for $32$ $2132.3$MB.
...
\paragraph{$\mathbf{Latency_{inference}}$}
\ref{fig:BatchSizeInferenceLatency}
...
The variance of $Latency_{network}$ across all images size and batch sizes, but even in case of cloud preprocessing with batch size $32$ and a $16$MP images, where in total more than $300$MB are transmitted to the cloud backend (see figure \ref{fig:BatchSizeTransmittedData}), the mean $Latency_{network}$ is below $200$ms. 
