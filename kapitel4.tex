...
Both edge preprocressing options, for either edge inference or cloud inference, have very similar memory consumption results with the cloud inference mean values being at most $5\%$ higher from the edge inference values for all image sizes.
...
Thus the $5\%$ overhead in memory can be explained by the additional \emph{PredictRequest} object that has to loaded into memory for cloud inference.
...
While the difference is only $2\%$ for an $224^2/299^2$ image, the difference grows to an $22.5\%$ decrease in memory for $16MP$ images ($2MP$: $1.1\%$, $4MP$: $7.9\%$, $8MP$: $9.9\%$).
