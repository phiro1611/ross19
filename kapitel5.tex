...
During the benchmarks, no other applicat,ions are running on either the edge or the cloud device.
...
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}[label = lst:network, caption = Network connection test to cloud-backend with iperf, escapeinside={(*}{*)}]
...
Another advantage of multiple linear regression is its interpretability.
...
Based on the previous defined regression formulas, we can now use the generated benchmark data to fit those models using the ordinary least squares approach.
...
The coefficient of the $ImageSize$ predictor for $\hat{Y}_{Cloud_{CloudPrepro}}$ is more than $3$ times higher than for the other models, confirming the high impact of cloud preprocessing on latency.
This coefficient is relatively similar between both edge preprocessing options $\hat{Y}_{Edge}$ and $\hat{Y}_{Cloud_{EdgePrepro}}$, as both use the same preprocessing method.
...
\paragraph{Confidence Intervals for Regression Parameters}
...
Out of the three models, only one includes a confidence interval, where the interval includes zero.
