...
Preprocessing large images is therefore mainly affected by the resizing process, not the pixel normalization or conversion to a \emph{ByteBuffer}, since these steps are performed after the resizing, thus having the same impact on performance as they would have on a $224^2/299^2$ image.
...
When differentiating between the different image sizes and inference modes the difference is always between $4-8\%$, except for cloud preprocessing of a $224^2/299^2$ image, where the difference rises to $10\%$ ($11.35$MB). This is due to the fact that for MobileNetV2 $224^2$ images are preprocessed and InceptionV4 $299^2$ images.
...
When comparing edge preprocessing to cloud preprocessing the latency gap for $224^2/299^2$ images is a factor of $20$ in favor of cloud preprocessing ($1.9/38.2$ms). For larger image sizes this gap shrinks, but still amounts $11\times$ for $16$MP images ($37.9/430.78$ms).
...
Even though cloud inference with preprocessing has the lowest preprocessing latency, this options remains the worst inference option for all except the $224^2/299^2$ images.
...
\caption[Edge vs. Cloud Inference for larger Batch Sizes:  $CPU_{preprocessing}$ - lower is better]{Edge vs. Cloud Inference for larger Batch Sizes:  $CPU_{preprocessing}$ - lower is better}
...
\caption[Edge vs. Cloud Inference for larger Batch Sizes:  $Memory_{preprocessing}$ - lower is better]{Edge vs. Cloud Inference for larger Batch Sizes:  $Memory_{preprocessing}$ - lower is better}
...
\caption[Edge vs. Cloud Inference for larger Batch Sizes:  $Latency_{preprocessing}$ - lower is better]{Edge vs. Cloud Inference for larger Batch Sizes:  $Latency_{preprocessing}$ - lower is better}
...
\caption[Edge vs. Cloud Inference for larger Batch Sizes:  $CPU_{inference}$ - lower is better]{Edge vs. Cloud Inference for larger Batch Sizes:  $CPU_{inference}$ - lower is better}
...
\caption[Edge vs. Cloud Inference for larger Batch Sizes:  $Memory_{inference}$ - lower is better]{Edge vs. Cloud Inference for larger Batch Sizes:  $Memory_{inference}$ - lower is better}
...
We believe the reason behind this is the reduced I/O cost in case of the smaller $224^2/299^2$ image, but for the larger images, this advantage vanishes as the un-preprocessed images causes higher I/O costs as well as higher preprocessing costs than the preprocessed counterparts.
...
\caption[Edge vs. Cloud Inference for larger Batch Sizes:  $Latency_{inference}$ - lower is better]{Edge vs. Cloud Inference for larger Batch Sizes:  $Latency_{inference}$ - lower is better}
...
\begin{itemize}
    ...
    \item As image and batch sizes grow $Memory_{preprocessing}$ gap between the two preprocessing options grows up to $842.2$MB, where edge preprocessing uses $2.88\times$ more memory than cloud preprocessing. 
...
\begin{equation*}
\begin{gathered}
\hat{Y}(X,\beta) = Latency_{total}^{pred}(X,\beta) = X \beta^\star = \begin{bmatrix} 1 &x_{1} & x_{2}& x_{p} \end{bmatrix} \begin{bmatrix}
           \beta_{0} \\
           \beta_{1} \\
           \beta_{2} \\
           \beta_{p}
...
\end{gathered}
\end{equation*}
where $\beta_0$ is the intercept, $(\beta_1,...,\beta_p)$ are the coefficients and $X$ is the vector with the influencing factors.
...
\begin{equation*}
\begin{gathered}
OptimalChoice(X) = \left\{\begin{array}{ll}
Edge  & \hat{Y}_{Edge} \leq \hat{Y}_{Cloud_{EdgePrepro}} \And \hat{Y}_{Cloud_{CloudPrepro}}\\
Cloud_{EdgePrepro}  &\hat{Y}_{Cloud_{EdgePrepro}} <   \hat{Y}_{Edge} \And \hat{Y}_{Cloud_{CloudPrepro}}\\
...
\begin{equation*}
\begin{split}
\hat{Y}_{Edge} &= 188.40440660014437 -103.5123906602889\cdot MobileNetV2 \\
           &+ 103.51239065520738\cdot InceptionV4 +2.523697773009001\mathrm{e}{-5}\cdot ImageSize
\end{split}
\end{equation*}
\begin{equation*}
\begin{split}
\hat{Y}_{Cloud_{EdgePrepro}} &= 
252.93401833789687 + MobileNetV2*-49.8078214129954\\
&+ InceptionV4*49.8078214174414+ ImageSize*8.990839081055035\mathrm{e}{-5}
\end{split}
\end{equation*}
\begin{equation*}
\begin{split}
\hat{Y}_{Cloud_{CloudPrepro}} &= 
202.82180883628232 + MobileNetV2*-38.22349157505442\\ &+InceptionV4*38.22349157378343+ ImageSize*2.3822579766343657\mathrm{e}{-5}
\end{split}
\end{equation*}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.99\textwidth]{./Bilder/Figure_1_temp.png}
